...
class Card {
    constructor(_card, _timer) {
        this.card = _card;
        this.timer = _timer;
    }

    updateTimer(_newTime) {
        this.timer.innerHTML = sixtyToZero(_newTime);
    }
...
// Create Card Objects
const daysCard = new Card(daysContainer, timeDays);
const hoursCard = new Card(hoursContainer, timeHours);
const minutesCard = new Card(minutesContainer, timeMinutes);
const secondsCard = new Card(secondsContainer, timeSeconds);
...
setInterval(() => {
    ...
    hoursCard= leftTime.h[0];
    minutesCard = leftTime.m[0];
    secondsCard = leftTime.s[0];
